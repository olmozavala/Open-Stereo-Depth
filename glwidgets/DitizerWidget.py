...
class DigitizerWidget(QGLWidget):
    ...
    def __init__(self, parent,leftImg, rightImg):
        ...
        self.dispAll = False # Define if we should display all the points or only the corresponding image
...
#                self.ptsManager.printPoints()
    ...
    def keyPressEvent(self,key):
        ...
        if( (key.text() == 'b') or (key.text() == 'B')):
            self.dispAll = not(self.dispAll)
...
#            self.resetVariables()
    ...
    def updateCamera(self):
        ...
        gluPerspective(fov, ratio , self.hnear,self.hfar)#Sets the perspective properties

        gluLookAt( 0,0,0,0,0,-1,0,1,0 ) #Position and angle of the camera
    ...
    def render(self):
        ...
        if( not (self.hideImg)):
            if( (self.displayMode == 2) or (self.displayMode == 3)):#Displays only the left or right image
                #Origin of the square containing the texture
                ox = -sizew/2 - self.sqrOffsetX
                oy = -sizeh/2 + self.sqrOffsetY
                if(self.displayMode == 2):# Here we deside which image to use as texture
